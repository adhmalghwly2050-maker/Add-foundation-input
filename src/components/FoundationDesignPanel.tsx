...
import FoundationDatabaseWorkspace from './FoundationDatabaseWorkspace';
...
export default function FoundationDesignPanel({
  ...
  const [activeTab, setActiveTab ] = useState<'database-workspace' | 'interactive' | 'reinforced-design' | 'reinforced-detailing' | 'autosize' | 'batch' | 'validation' | 'strip-footing' | 'classification' | 'settlement'>('database-workspace');
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center border-b border-border pb-1">
        <div className="flex gap-2">
          <button
            onClick={() => setActiveTab('database-workspace')}
            className={`pb-2 px-3 text-xs font-semibold border-b-2 transition-all flex items-center gap-1 shrink-0 ${
              activeTab === 'database-workspace'
                ? 'border-rose-600 text-rose-600 font-bold'
                : 'border-transparent text-muted-foreground hover:text-foreground'
            }`}
          >
            📊 قاعدة وربط الأساسات (Data Model)
          </button>
          <button
          ...
          <button
            ...
            className={`pb-2 px-3 text-xs font-semibold border-b-2 transition-all shrink-0 ${
              activeTab === 'classification'
                ? 'border-blue-600 text-blue-600 font-bold'
                : 'border-transparent text-muted-foreground hover:text-foreground'
            }`}
          >
            محرك تصنيف وتوحيد القواعد (Group Engine) ⚡
          </button>
          <button
          ...
          >
            تحليل هبوط أساسات التربة (Settlement Analysis) 📊
          </button>
        </div>
      </div>

      {/* ── TAB: FOUNDATION DATABASE WORKSPACE & DATAMODEL ── */}
      {activeTab === 'database-workspace' && (
        <FoundationDatabaseWorkspace
          columns={columns}
