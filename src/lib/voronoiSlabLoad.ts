...
export function computeVoronoiSlabLoad(
  ...
  let spanDirection: 'x' | 'y' | 'two-way' = 'two-way';
  const sType = slab.type || 'solid';
  const sDirection = slab.direction || 'auto';
  const sRibDirection = slab.ribDirection || 'x';

  if (sType === 'ribbed') {
    spanDirection = sRibDirection === 'y' ? 'y' : 'x';
  } else {
    if (sDirection === 'one-way-x') {
      spanDirection = 'x';
    } else if (sDirection === 'one-way-y') {
      spanDirection = 'y';
    } else if (sDirection === 'two-way') {
      spanDirection = 'two-way';
    } else {
      // auto: ACI aspect ratio beta = long / short > 2
      const beta = Math.max(rangeX, rangeY) / Math.max(1, Math.min(rangeX, rangeY));
      if (beta > 2) {
        spanDirection = rangeX < rangeY ? 'x' : 'y'; // spans along the shorter axis
      } else {
        spanDirection = 'two-way';
      }
    }
  ...
  // Filter supporting beams based on spanning direction
  let allowedBeamsIndices: number[] = [];
  for (let k = 0; k < nBeams; k++) {
    ...
    if (spanDirection === 'x') {
      // Spans along X: loads support on vertical/Y-running beams (dy > dx)
      if (isVertical) allowedBeamsIndices.push(k);
    } else if (spanDirection === 'y') {
      // Spans along Y: loads support on horizontal/X-running beams (dx >= dy)
      if (isHorizontal) allowedBeamsIndices.push(k);
    } else {
      // Two way: all beams can receive loads
      allowedBeamsIndices.push(k);
    }
  }

  // Fallback: if no beams match the condition
  if (allowedBeamsIndices.length === 0) {
    allowedBeamsIndices = Array.from({ length: nBeams }, (_, i) => i);
  }
  ...
  for (let i = 0; i < nx; i++) {
    for (let j = 0; j < ny; j++) {
      ...
      // Find nearest allowed beam
      let minDist = Infinity, nearIdx = -1, nearT = 0;
      for (const k of allowedBeamsIndices) {
        const b = supportingBeams[k];
