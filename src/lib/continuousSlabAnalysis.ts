...
export interface SpanResult {
  ...
  As_min: number;      // mm²/m
  isRibbed?: boolean;
  Mneg_left_per_rib?: number;
  Mpos_per_rib?: number;
  Mneg_right_per_rib?: number;
  Vu_left_per_rib?: number;
  Vu_right_per_rib?: number;
  As_neg_left_per_rib?: number;
  As_pos_per_rib?: number;
  As_neg_right_per_rib?: number;
  As_min_per_rib?: number;
}
...
export function analyzeContinuousStrip(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const isSpanRibbed = isRibbed || matchingSlab?.slabType === 'one_way_ribbed' || matchingSlab?.type === 'one_way_ribbed';
    const ribCount = 2; // contains 2 ribs per meter width
    ...
    spanResults.push({
      ...
      As_min: AsMin,
      isRibbed: isSpanRibbed,
      Mneg_left_per_rib: isSpanRibbed ? Mneg_left / ribCount : undefined,
      Mpos_per_rib: isSpanRibbed ? Mpos / ribCount : undefined,
      Mneg_right_per_rib: isSpanRibbed ? Mneg_right / ribCount : undefined,
      Vu_left_per_rib: isSpanRibbed ? Vu_left / ribCount : undefined,
      Vu_right_per_rib: isSpanRibbed ? Vu_right / ribCount : undefined,
      As_neg_left_per_rib: isSpanRibbed ? calcAs(Mneg_left) / ribCount : undefined,
      As_pos_per_rib: isSpanRibbed ? calcAs(Mpos) / ribCount : undefined,
      As_neg_right_per_rib: isSpanRibbed ? calcAs(Mneg_right) / ribCount : undefined,
      As_min_per_rib: isSpanRibbed ? AsMin / ribCount : undefined,
    });
  }
  ...
  return {
    ...
    isRibbed: isRibbed || matchingSlab?.slabType === 'one_way_ribbed' || matchingSlab?.type === 'one_way_ribbed',
  };
}
